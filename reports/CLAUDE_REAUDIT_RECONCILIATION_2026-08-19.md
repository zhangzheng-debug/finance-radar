...
| FR-RA-004 | “来源留出”只有文档文字 | 同源数据可同时用于开发和盲测 | 归一化 source family；apply 至少要求一个历史语料完全未出现的来源家族，并绑定授权 | 仓库已修；无 held-out family 时诚实 BLOCKED |
...
- `python -m pytest -q`：`723 passed, 5 skipped`
- `git diff --check`：PASS
...
本分支应以 Draft PR 进入评审。CI、全量测试、diff check、shell/YAML 门禁全部通过后再考虑合并。合并不自动触发模型晋级、真人冻结、AWS 部署或公共资产删除。
