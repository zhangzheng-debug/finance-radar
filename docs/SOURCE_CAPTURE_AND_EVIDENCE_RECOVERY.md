...
只有一个事件完全没有任何证据关系时，Public 才可以显示一段独立 AI 解读，帮助用户理解英文或压缩后的 API 文本。页面必须明确说明“因为事件完全没有证据，所以启动 AI 自动解释”，并把它与证据和正式事实严格分层：

- 任意 `event_evidence` 行都会立即关闭并隐藏外部 AI；存在 P0/P1 URL 时优先重抓原始来源，不调用模型；
- 无缓存时只显示局部等待状态，不生成或展示确定性伪预览；已配置的后台任务可以生成收据绑定的 DeepSeek 辅助解读，但 Public 浏览本身绝不触发外部调用；
- 普通浏览不会触发外部调用或账单；
- 模型输入必须经过白名单和限长，且来源文本一律视为不可信输入；
- 所有逐字引用必须能在当前捕获文本中找到；
- 结果绑定当前事件版本、来源 revision、内容哈希、采集收据、合同、模型和提示词哈希；
