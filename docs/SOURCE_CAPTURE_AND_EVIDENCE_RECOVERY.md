...
`0 条可引用支持证据` 不等于 API 输入为空。Public 的排除档案可以显示经过限长和脱敏的来源记录，但必须同时标注“未核验、非证据”；不得公开聚合源的评分、等级、多空信号或完整 `raw_json`。

## 2. Public 行为

- 默认事件主流仍只展示 `reader_ready` 事件，不让聚合噪声重新污染正式阅读流。
- 用户显式进入“已排除”档案时，可看到仍有保留采集记录的 rejected 事件。
- 页面分别显示“采集来源记录 N”与“可引用支持证据 M”。
...
Public 可以在“未核验、非证据”的来源卡中显示一段独立解读，帮助用户理解英文或压缩后的 API 文本。该解读与证据和正式事实严格分层：
