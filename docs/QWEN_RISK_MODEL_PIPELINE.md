...
- 缺失、过期、未获批或无法判断的模型结果不在 Public 生成占位、徽标或解释区；事件与来源材料仍可立即浏览。
...
- 公开事件详情预留固定的“方向 / 做空重大性 / 强度”槽位。模型未获准时仅显示空值和“模型接口已预留”；获准后由同一 `semantic_assessment` 合同直接填充，不以旧 RiskRouter、规则门或 DeepSeek 结果冒充千问输出。
