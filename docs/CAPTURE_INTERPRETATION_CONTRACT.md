...
- 一句话中文解释（通常 25–90 个中文字符）；
- 最多两条彼此不同的原文主张，以及逐字引用；
- 行为主体、受影响资产及其原文引用；
- 语气阶段（已发生、已宣布、拟议、条件性、否认、评论或不清楚）；
- 来源形态（单一事件、多主题综述、评论或未知）；
- 是否发现疑似提示词注入。

服务器会拒绝多余字段、虚构引用、未出现在来源中的资产或数值、非法阶段、过长字段和可疑注入文本，并删除重复或无实质内容的要点。公开边界由服务器固定为一句“AI仅解释来源文本，不参与事件评级或价格判断”，不再要求模型反复生成“没有证明什么”和“还缺什么”的清单。正式状态说明、安全标记、事件版本、收据哈希和提示词哈希均由服务器写入，模型无权提供或覆盖。

当前生成合同为 `capture-interpretation-prompt-v4`。过渡期内，页面可以继续读取与当前捕获收据严格绑定、且通过质量门的 v3 缓存；同一收据若已有 v4，则始终优先 v4。这样升级提示词不会让页面突然空白，旧运行记录仍保留用于审计。
