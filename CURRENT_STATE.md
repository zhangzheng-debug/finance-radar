...
## 2026-09-01 `2026.09.01.3` 模型执行链恢复与读者请求保留

千问模型服务本身维持回环运行，worker 改为每 15 秒按墙钟重新触发；即使某轮非零退出，timer 仍必须
保有下一次执行时间。每轮最多处理 4 条、单次模型请求最多 30 秒。Public 当前阅读事件使用独立标记的
FIFO 优先级，先于自动重试和后台最近/公平扫描执行；队列容量被自动重试占满时，Public 请求仍可抢占
一个后台位置。只有模型调用已真正领取时显示运行态，结果持久化后才展示方向、重大性和强弱。

DeepSeek 继续只处理没有 reader-eligible 证据、但保留了可读捕获文本的边界事件。Public 任务若因来源
内容变化而过期，旧身份保留失败审计，当前输入会在同一事务中重新排队；若后台任务已经触发日预算
延后，Public 打开事件可使用预留的每日 100 次 / 1 CNY 份额，但仍受总计 500 次 / 5 CNY 的硬上限。
普通 provider 退避不会被 Public 绕过。页面只显示紧凑的真实生命周期，终态可由用户显式重新生成。

部署仍是完整事务：先建立并验证新备份，再安装新归档和 systemd 单元；验收除 API、Web 与 timer 状态
外，还要求千问 timer 的 `NextElapseUSecRealtime` 非空，并在 AWS 现场验证真实事件从排队、领取到
结果落库。两条模型链均为只读研究辅助，不改变事实、证据、排序、行情审计或任何交易行为。
...
模型上线。它在冻结开发验证集上的四字段完全一致率为 0.8947，重大性 Macro-F1 为 0.9394，
极性 Macro-F1 为 0.8175，优先事件召回为 1.0000，非优先误报率为 0.0417。该成绩属于固定的
57 条开发验证，不改称独立人类盲测，也不宣称为 95% 全任务准确率。

运行时使用 Q4_K_M 基座 GGUF 与 v3 checkpoint-40 LoRA。基座、LoRA、原始 adapter、训练清单、
模型报告、混合报告与 owner 发布策略全部由 SHA-256 绑定；任一文件变化，模型服务拒绝启动。
Public 只显示当前事件版本、当前输入和已批准模型完全匹配的持久化结果；没有结果时不显示空卡、
等待或“接口预留”文案。正面与中性结果不再被渲染成“负面强度低”。

这一语义结果只回答来源文本表达的方向、重大性与负面强度，不确认事件真假，不改变事实、证据、
价格或排序，也不进入任何交易路径。模型仅绑定回环地址，后台按最近事件与公平队列持续补齐结果。

## 2026-08-30 `2026.08.30.1` AI 仲裁研究基线收口

PR #47 的 AI 辅助仲裁、数据隔离、泄漏防护、训练与评估脚本、冻结报告和可复算收据已
并入主线，作为后续模型工作的正式研究基线。风险路由器达到多切分影子候选门槛，职责仍
仅限于优先级研究，不改变事件事实、证据状态、公共排序或交易边界。

千问语义路线没有因此晋级生产：旧版 1.5B 混合方案只达到开发期影子候选，新鲜 DEV 上的
3B 本地训练受硬件吞吐阻塞，CPU 后备候选也未通过重大性、极性和优先事件召回门槛。生产
千问开关、Public 语义评级和模型服务继续保持关闭；owner holdout 仍封存，未把 AI 仲裁
样本冒充真人金标。下一次晋级必须先冻结新的独立盲集，再进行一次性评估和单独发布批准。

本版本不修改生产数据合同、数据库 Schema、事件证据呈现、价格审计或服务启用集合，也不
接入任何交易能力。部署验收仍须绑定本次 commit、标准备份收据、systemd/Nginx 状态和公网
只读检查。

## 2026-08-29 `2026.08.29.3` 历史原文重接纳与常数时间健康探针候选

公共健康探针改为读取 overview 任务原子发布的有界快照，不再在每次请求中扫描生产级
SQLite 账本或反序列化大型备份组件清单；需要当前数据库计数的诊断保留在管理员鉴权的
`/api/v1/health/deep`。公开事件、详情和价格映射契约不因此改变。

历史 P0/P1 重接纳新增收益、指引、持续经营、回购授权扩张、破产、违约、重组、反向拆股、
召回、临床进展和 SPAC IPO 等窄规则。SEC 文件里的 Company/Registrant 代词只有在文件 CIK
与规范主体 CIK 精确一致时才可绑定；句子仍必须明确表达允许的动作和对象。错误事件族、否定、
预测、来源修订不一致和时间不精确继续被挡住，不会因“有链接”自动变成事实。

资产注册表移除供应商不可用的 PGAL、EGPT、PAK 与 NGE，分别改用可获取的 FEZ 或 EEM 广义
代理并明确标注代理范围。本候选全量回归为 `1334 passed, 6 skipped`，组合回归为
`157 passed`，`compileall` 与 `git diff --check` 均通过。千问生产开关保持关闭；720 条双人
金标、训练、盲测与发布批准仍是唯一明确保留的后续工作。
