...
# These phrases are not ordinary contemporaneous market commentary.  They are
# generated by historical outcome-mining jobs and therefore leak the answer
# construction into otherwise provider-only source packets.  Candidate rows
# containing them are excluded before selection; the numeric value is never
# used for ranking, stratification, or labeling.
PROHIBITED_SUPERVISION_TEXT_PATTERNS: tuple[tuple[str, str], ...] = (
    (
        "FORWARD_RETURN_THRESHOLD",
        r"\bret_(?:1d|3d|5d|10d|20d|21d)\s*(?:<=|>=|=)\s*-?\d",
    ),
    (
        "FORWARD_CRASH_CANDIDATE",
        r"\b(?:one|three|five|ten|twenty[_ -]?one|1|3|5|10|20|21)"
        r"[_ -]?day[_ -]?crash\s+candidate\b",
    ),
    ("VOLUME_CRASH_CANDIDATE", r"\bvolume[_ -]?crash\s+candidate\b"),
    ("OUTCOME_VOLUME_RATIO", r"\bvolume_ratio\s*="),
)
_PROHIBITED_SUPERVISION_REGEXES = tuple(
    (name, re.compile(pattern, re.IGNORECASE))
...
def _prohibited_supervision_text_reasons(value: Any) -> list[str]:
    matches = {
